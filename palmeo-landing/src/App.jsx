...
function LogoMarquee() {
  ...
  return (
    <div style={{ overflow: 'hidden', position: 'relative' }}>
      <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: 60, background: 'linear-gradient(to right, #08090e, transparent)', zIndex: 2, pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', right: 0, top: 0, bottom: 0, width: 60, background: 'linear-gradient(to left, #08090e, transparent)', zIndex: 2, pointerEvents: 'none' }} />
      <motion.div
        animate={{ x: ['0%', '-50%'] }}
        transition={{ duration: 30, repeat: Infinity, ease: 'linear' }}
        style={{ display: 'flex', gap: 10, width: 'max-content', padding: '8px 0' }}
      >
        {doubled.map(({ name, color, bg }, i) => (
          <div key={i} style={{
            ...
            <span style={{ fontSize: 12, fontWeight: 600, color: '#d4d4d8' }}>{name}</span>
          </div>
        ))}
      </motion.div>
    </div>
...
const MODELS = [
  {
    ...
    points: ['Import de vos fichiers Excel existants', 'Structuration et normalisation des données', 'Compatible avec tous les formats courants', 'Aucun ERP requis'],
    accent: '#60a5fa', featured: false,
  },
  {
    ...
    points: ['Synchronisation automatique des données', 'Aucune saisie manuelle', 'Mise à jour en temps réel', 'Compatible avec les principaux ERP du marché'],
  ...
  },
  {
    ...
    points: ['Analyse complète de vos besoins', 'Développement dédié à votre huilerie', 'Modules et rapports personnalisés', 'Accompagnement de bout en bout'],
    accent: '#c084fc', featured: false,
  },
]
...
function DashboardDesktopMockup() {
  return (
    <div style={{ background: 'rgba(10,11,15,0.95)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', boxShadow: '0 32px 80px rgba(0,0,0,0.6)' }}>
      <div style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,0.02)' }}>
        <div style={{ display: 'flex', gap: 5 }}>
          {['#ff5f57','#febc2e','#28c840'].map(c => <div key={c} style={{ width: 11, height: 11, borderRadius: '50%', background: c }} />)}
        </div>
        <div style={{ flex: 1, background: 'rgba(255,255,255,0.05)', borderRadius: 5, height: 22, maxWidth: 220, display: 'flex', alignItems: 'center', paddingLeft: 8, fontSize: 10, color: '#3f3f46' }}>app.palmeo.co</div>
      </div>
      <div style={{ display: 'flex', height: 340 }}>
        <div style={{ width: 130, borderRight: '1px solid rgba(255,255,255,0.05)', padding: '12px 8px', display: 'flex', flexDirection: 'column', gap: 4, background: 'rgba(255,255,255,0.01)' }}>
          ...
          {[{icon:'📊',label:"Vue d'ensemble",active:true},{icon:'🌴',label:'Production',active:false},{icon:'💰',label:'Revenus',active:false},{icon:'💸',label:'Charges',active:false},{icon:'🚚',label:'Fournisseurs',active:false}].map(({icon,label,active}) => (
            <div key={label} style={{ display:'flex',alignItems:'center',gap:6,padding:'6px 8px',borderRadius:7,fontSize:11,background:active?'rgba(74,222,128,0.1)':'transparent',color:active?'#4ade80':'#71717a' }}>
              <span style={{fontSize:12}}>{icon}</span><span style={{fontWeight:active?600:400}}>{label}</span>
            </div>
          ))}
          <div style={{ marginTop: 'auto', padding: '6px 8px' }}>
            <div style={{ fontSize: 10, color: '#3f3f46', marginBottom: 6 }}>Notifications</div>
            <div style={{ display:'flex',alignItems:'center',gap:5,background:'rgba(74,222,128,0.1)',border:'1px solid rgba(74,222,128,0.2)',borderRadius:6,padding:'5px 7px',fontSize:10,color:'#4ade80' }}>🔔 Notifs actives</div>
          </div>
        </div>
        <div style={{ flex: 1, padding: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
            ...
            <div style={{display:'flex',gap:6}}>
              {['FCFA','EUR'].map((c,i) => (
                <div key={c} style={{padding:'3px 8px',borderRadius:5,fontSize:10,fontWeight:600,background:i===0?'rgba(74,222,128,0.15)':'transparent',border:`1px solid ${i===0?'rgba(74,222,128,0.3)':'rgba(255,255,255,0.08)'}`,color:i===0?'#4ade80':'#52525b'}}>{c}</div>
              ))}
            </div>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:8 }}>
            {[{label:'TE moyen',value:'21.3%',delta:'+0.8%',ok:true},{label:'Régimes reçus',value:'12 450 T',delta:'ce mois',ok:null},{label:'Huile produite',value:'2 580 T',delta:'+3.2% vs M-1',ok:true},{label:'C.A. HT',value:'487 M',delta:'FCFA',ok:null}].map(({label,value,delta,ok}) => (
              <div key={label} style={{background:'rgba(255,255,255,0.03)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:8,padding:'10px 10px'}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr',gap:8,flex:1}}>
            <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:8,padding:10}}>
              ...
              <svg viewBox="0 0 400 90" width="100%" height="80" preserveAspectRatio="none">
                <defs><linearGradient id="g2" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#16a34a" stopOpacity="0.35"/><stop offset="100%" stopColor="#16a34a" stopOpacity="0"/></linearGradient></defs>
            ...
            <div style={{background:'rgba(255,255,255,0.02)',border:'1px solid rgba(255,255,255,0.05)',borderRadius:8,padding:10,display:'flex',flexDirection:'column',gap:8}}>
              <div style={{fontSize:10,color:'#52525b'}}>P&L — Juin 2026</div>
              {[{label:'Revenus',value:'487 M',bar:100,color:'#4ade80'},{label:'Charges',value:'312 M',bar:64,color:'#f87171'},{label:'Marge nette',value:'175 M',bar:36,color:'#60a5fa'}].map(({label,value,bar,color}) => (
                <div key={label}>
                  <div style={{display:'flex',justifyContent:'space-between',fontSize:9,color:'#71717a',marginBottom:3}}><span>{label}</span><span style={{color:'#d4d4d8'}}>{value} FCFA</span></div>
                  <div style={{height:4,background:'rgba(255,255,255,0.06)',borderRadius:2}}><div style={{height:'100%',width:`${bar}%`,background:color,borderRadius:2,opacity:0.8}}/></div>
                </div>
              ))}
            </div>
          </div>
...
function PhoneMockup({ scale = 1 }) {
  return (
    <div style={{
      ...
      width: 220 * scale, boxShadow: '0 32px 80px rgba(0,0,0,0.7)',
      border: '1px solid rgba(255,255,255,0.1)',
    }}>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 8 }}>
        <div style={{ width: 80 * scale, height: 22 * scale, background: '#000', borderRadius: 11, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ width: 10, height: 10, background: '#1c1c1e', borderRadius: '50%' }} />
        </div>
      </div>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10 * scale, color: '#636366', marginBottom: 10, padding: '0 6px' }}>
        <span style={{ fontWeight: 600 }}>19:02</span><span>▐▐▐ 🔋</span>
      </div>
      <div style={{ padding: '0 4px' }}>
        <div style={{ background: 'rgba(44,44,46,0.95)', backdropFilter: 'blur(12px)', borderRadius: 14, padding: `${10 * scale}px ${12 * scale}px`, border: '1px solid rgba(255,255,255,0.08)', boxShadow: '0 8px 24px rgba(0,0,0,0.4)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
            <div style={{ width: 28 * scale, height: 28 * scale, borderRadius: 7, flexShrink: 0, background: 'linear-gradient(135deg,#16a34a,#4ade80)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12 * scale, fontWeight: 700, color: '#fff' }}>P</div>
            <div>
              <div style={{ fontSize: 9 * scale, color: '#636366' }}>palmeo · maintenant</div>
              <div style={{ fontSize: 11 * scale, fontWeight: 600, color: '#fff' }}>APO — 18 juin  ✓ TE 21.3%</div>
            </div>
          </div>
          <div style={{ fontSize: 10 * scale, color: '#aeaeb2', lineHeight: 1.55 }}>
        ...
        <div style={{ marginTop: 10, background: 'rgba(28,28,30,0.9)', borderRadius: 12, padding: 10 * scale, border: '1px solid rgba(255,255,255,0.06)' }}>
          <div style={{ fontSize: 9 * scale, color: '#52525b', marginBottom: 6 }}>Palmeo · Vue rapide</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
            {[{label:'TE',value:'21.3%',color:'#4ade80'},{label:'Huile',value:'168 T',color:'#fff'},{label:'C.A.',value:'487M',color:'#fff'},{label:'Marge',value:'35.9%',color:'#60a5fa'}].map(({label,value,color}) => (
              <div key={label} style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 7, padding: `${6 * scale}px ${8 * scale}px` }}>
                <div style={{ fontSize: 8 * scale, color: '#52525b' }}>{label}</div>
                <div style={{ fontSize: 13 * scale, fontWeight: 700, color }}>{value}</div>
              </div>
            ))}
          </div>
        </div>
...
function DemoModal({ onClose }) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', padding: '0', background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)' }}
      onClick={onClose}
    >
      <motion.div
        ...
        style={{ width: '100%', maxWidth: 480, padding: '32px 24px 40px', borderRadius: '20px 20px 0 0', position: 'relative', background: '#111318', border: '1px solid rgba(255,255,255,0.1)' }}
...
/* ─── Mobile nav menu ─── */
function MobileMenu({ onClose, onDemo }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      style={{ position: 'fixed', inset: 0, zIndex: 40, background: 'rgba(8,9,14,0.97)', backdropFilter: 'blur(12px)', display: 'flex', flexDirection: 'column', padding: '80px 32px 40px' }}
      onClick={onClose}
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {[['#integrations','Intégrations'],['#models','Offres'],['#features','Fonctionnalités'],['https://app.palmeo.co','Se connecter']].map(([href, label]) => (
          <a key={href} href={href} onClick={onClose} style={{ color: '#d4d4d8', fontSize: 24, fontWeight: 600, textDecoration: 'none', padding: '12px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>{label}</a>
        ))}
      </div>
      <button className="btn-primary" style={{ marginTop: 40, fontSize: 16, padding: '16px' }} onClick={() => { onClose(); onDemo(); }}>
...
export default function App() {
  ...
  return (
    <div style={{ background: '#08090e', minHeight: '100vh' }}>
      ...
      {/* Orbs */}
      <div className="orb" style={{ width: isMobile ? 300 : 600, height: isMobile ? 300 : 600, top: -100, left: '50%', transform: 'translateX(-50%)', background: 'radial-gradient(circle, rgba(22,163,74,0.15) 0%, transparent 70%)' }} />
      <div className="orb" style={{ width: 400, height: 400, bottom: '20%', right: '-20%', background: 'radial-gradient(circle, rgba(251,191,36,0.08) 0%, transparent 70%)' }} />
      ...
      <motion.nav
        ...
        style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 30, borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(8,9,14,0.85)', backdropFilter: 'blur(16px)' }}
      >
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: isMobile ? '16px 20px' : '16px 28px', maxWidth: 1100, margin: '0 auto' }}>
          ...
          {isMobile ? (
            <button
              ...
              style={{ background: 'none', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 8, padding: '8px', cursor: 'pointer', color: '#fff', display: 'flex', alignItems: 'center' }}
            >
              <Menu size={20} />
            </button>
          ) : (
      ...
      <AnimatePresence>
        {showMobileMenu && (
          <MobileMenu onClose={() => setShowMobileMenu(false)} onDemo={() => setShowModal(true)} />
        )}
      </AnimatePresence>

      {/* ══════════ HERO ══════════ */}
      <section style={{ position: 'relative', zIndex: 10, textAlign: 'center', padding: isMobile ? '100px 20px 60px' : '120px 24px 80px', maxWidth: 1060, margin: '0 auto' }}>
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          className="tag" style={{ display: 'inline-flex', marginBottom: 24 }}>
        ...
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2, ease: [0.22,1,0.36,1] }}
          ...
          style={{ fontSize: isMobile ? '38px' : 'clamp(40px,6vw,68px)', fontWeight: 800, letterSpacing: isMobile ? '-1px' : '-2px', lineHeight: 1.1, marginBottom: 20 }}>
        ...
        <motion.p initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55, delay: 0.32 }}
          style={{ color: '#71717a', fontSize: isMobile ? 15 : 18, lineHeight: 1.7, maxWidth: 560, margin: '0 auto 32px' }}>
          Taux d'extraction, régimes, P&L mensuel, alertes IA — toutes vos données de production sur un seul tableau de bord, depuis n'importe quel appareil.
        </motion.p>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.42 }}
          ...
          <button className="btn-primary" style={{ fontSize: isMobile ? 15 : 16, padding: isMobile ? '13px 22px' : '14px 28px', width: isMobile ? '100%' : 'auto' }} onClick={() => setShowModal(true)}>
            Demander une démo <ChevronRight size={16} />
          </button>
          <a href="https://app.palmeo.co" className="btn-secondary" style={{ fontSize: isMobile ? 15 : 16, padding: isMobile ? '13px 22px' : '14px 28px', width: isMobile ? '100%' : 'auto', textAlign: 'center' }}>Voir l'application</a>
        </motion.div>
        ...
        <motion.div
          ...
          style={{ position: 'relative', maxWidth: 1060, margin: isMobile ? '40px auto 0' : '60px auto 0' }}
        >
          {isMobile ? (
            /* Mobile: phone mockup centré */
            ...
            /* Desktop: dashboard + phone côte à côte */
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: 24, alignItems: 'center' }}>
              <div style={{ position: 'relative', minWidth: 0 }}>
                ...
                <div style={{ position: 'relative' }}><DashboardDesktopMockup /></div>
              </div>
      ...
      {/* ══════════ STATS ══════════ */}
      <section id="stats" style={{ position: 'relative', zIndex: 10, borderTop: '1px solid rgba(255,255,255,0.05)', borderBottom: '1px solid rgba(255,255,255,0.05)', padding: isMobile ? '48px 24px' : '64px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3,1fr)', gap: isMobile ? 32 : 40, textAlign: 'center' }}>
      ...
      {/* ══════════ INTEGRATIONS ══════════ */}
      <section id="integrations" style={{ position: 'relative', zIndex: 10, padding: isMobile ? '60px 0' : '80px 0' }}>
        <AnimateIn style={{ textAlign: 'center', marginBottom: 36, padding: '0 20px' }}>
          <div className="tag" style={{ display: 'inline-flex', marginBottom: 16 }}>
            <Database size={11} /> Sources de données
          </div>
          <h2 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '24px' : 'clamp(24px,3.5vw,36px)', letterSpacing: -0.8, marginBottom: 12 }}>
      ...
      {/* ══════════ 3 MODÈLES ══════════ */}
      <section id="models" style={{ position: 'relative', zIndex: 10, padding: isMobile ? '0 16px 72px' : '0 24px 96px', maxWidth: 1100, margin: '0 auto' }}>
        <AnimateIn style={{ textAlign: 'center', marginBottom: 48 }}>
          <div className="tag" style={{ display: 'inline-flex', marginBottom: 16 }}>
            <Puzzle size={11} /> Nos offres
          </div>
          <h2 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '26px' : 'clamp(26px,4vw,40px)', letterSpacing: -1, marginBottom: 12 }}>
          ...
          {MODELS.map(({ icon, badge, title, desc, sources, points, accent, featured }, i) => (
            <AnimateIn key={badge} i={i}>
              <div style={{
                ...
                borderRadius: 20, padding: isMobile ? 22 : 28, display: 'flex', flexDirection: 'column',
                boxShadow: featured ? `0 0 0 1px ${accent}20, 0 8px 40px ${accent}10` : 'none',
                marginTop: featured && !isMobile ? 0 : 0,
              }}>
                {featured && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: `${accent}15`, border: `1px solid ${accent}30`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{icon}</div>
                  <div style={{ fontSize: 11, fontWeight: 700, color: accent, letterSpacing: 0.5, textTransform: 'uppercase' }}>{badge}</div>
                </div>
                <h3 style={{ color: '#fff', fontWeight: 700, fontSize: 17, lineHeight: 1.3, marginBottom: 10 }}>{title}</h3>
                <p style={{ color: '#71717a', fontSize: 14, lineHeight: 1.65, marginBottom: 18 }}>{desc}</p>
                ...
                <ul style={{ listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 9, marginTop: 'auto', marginBottom: 20 }}>
                  {points.map(pt => (
                    <li key={pt} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: '#d4d4d8' }}>
                      <Check size={14} style={{ color: accent, flexShrink: 0, marginTop: 2 }} />{pt}
                    </li>
                  ))}
                </ul>
                <button onClick={() => setShowModal(true)} style={{ width: '100%', padding: '11px 0', background: featured ? `linear-gradient(135deg,${accent}cc,${accent}88)` : 'transparent', border: `1px solid ${accent}40`, borderRadius: 10, cursor: 'pointer', color: featured ? '#fff' : accent, fontSize: 14, fontWeight: 600, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, transition: 'opacity 0.2s' }}
                  onMouseEnter={e => e.currentTarget.style.opacity = '0.8'}
                  onMouseLeave={e => e.currentTarget.style.opacity = '1'}
                >En savoir plus <ArrowRight size={14} /></button>
              </div>
            </AnimateIn>
          ))}
        </div>
      ...
      {/* ══════════ FEATURES ══════════ */}
      <section id="features" style={{ position: 'relative', zIndex: 10, padding: isMobile ? '0 16px 72px' : '0 24px 96px', maxWidth: 1100, margin: '0 auto' }}>
        <AnimateIn style={{ textAlign: 'center', marginBottom: 48 }}>
          <div className="tag" style={{ display: 'inline-flex', marginBottom: 16 }}>
            <TrendingUp size={11} /> Fonctionnalités
          </div>
          <h2 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '24px' : 'clamp(26px,4vw,40px)', letterSpacing: -1, marginBottom: 12 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit,minmax(300px,1fr))', gap: isMobile ? 12 : 16 }}>
          {FEATURES.map(({ icon, title, desc }, i) => (
            <AnimateIn key={title} i={i % 3} className="glass-card" style={{ padding: isMobile ? 20 : 24 }}>
      ...
      {/* ══════════ PUSH NOTIF ══════════ */}
      <section style={{ position: 'relative', zIndex: 10, padding: isMobile ? '0 20px 72px' : '0 24px 96px', maxWidth: 1000, margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: isMobile ? 48 : 60, alignItems: 'center' }}>
          <AnimateIn>
            <div className="tag" style={{ marginBottom: 20 }}>
              <Bell size={11} /> Push + IA
            </div>
            <h2 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '26px' : 'clamp(22px,3vw,34px)', letterSpacing: -0.8, marginBottom: 16 }}>
              Restez informé,<br /><span className="gradient-text">analysé par l'IA</span>
            </h2>
          ...
          <AnimateIn i={1} style={{ display: 'flex', justifyContent: 'center' }}>
            <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' }}>
              <div style={{ background: '#1c1c1e', borderRadius: 24, padding: 16, width: isMobile ? 280 : 290, boxShadow: '0 24px 80px rgba(0,0,0,0.6)', border: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      {/* ══════════ CTA ══════════ */}
      <section style={{ position: 'relative', zIndex: 10, padding: isMobile ? '0 16px 72px' : '0 24px 96px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto', textAlign: 'center' }}>
          <AnimateIn>
            <div style={{ position: 'relative', padding: isMobile ? '48px 24px' : '64px 40px', borderRadius: 24, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden' }}>
              ...
              <div className="tag" style={{ display: 'inline-flex', marginBottom: 20, position: 'relative' }}>
                <Mail size={11} /> Démo gratuite
              </div>
              <h2 style={{ color: '#fff', fontWeight: 800, fontSize: isMobile ? '26px' : 'clamp(26px,4vw,38px)', letterSpacing: -1, marginBottom: 14, position: 'relative' }}>
                Prêt à piloter votre huilerie<br /><span className="gradient-text">autrement ?</span>
              </h2>
              <p style={{ color: '#71717a', lineHeight: 1.7, marginBottom: 28, position: 'relative', fontSize: 15 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 12, flexWrap: 'wrap', position: 'relative' }}>
                <button className="btn-primary" style={{ fontSize: 16, padding: '14px 28px', width: isMobile ? '100%' : 'auto' }} onClick={() => setShowModal(true)}>
                  Demander une démo <ChevronRight size={16} />
                </button>
                <a href="https://app.palmeo.co" className="btn-secondary" style={{ fontSize: 15, width: isMobile ? '100%' : 'auto', textAlign: 'center' }}>Accéder à l'app →</a>
              </div>
            </div>
          </AnimateIn>
        </div>
      ...
      {/* ══════════ FOOTER ══════════ */}
      <footer style={{ position: 'relative', zIndex: 10, borderTop: '1px solid rgba(255,255,255,0.05)', padding: isMobile ? '28px 20px' : '36px 24px' }}>
        <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 16, fontSize: 13, color: '#3f3f46' }}>
